...
the project's review history (through Round 74, **2026-07-10** — the archive's round *numbering* runs to 74
while it holds 71 rounds on record, so this is the same archive every other page counts as
"71 review rounds, 3,298 findings"), the
covered-class analysis shows for this convention: **7 findings before mechanization, 0
after.**
...
[`core/companion/scripts/run_checks/grep.py:190`](../core/companion/scripts/run_checks/grep.py)
...
the part of Sysop that installs on its own: [loop mode](./loop-mode.md), the recommended first
install, for running exactly this against your project while your own merge workflow stays
put. The
