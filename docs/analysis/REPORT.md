...
**Date:** 2026-06-09 (revised same day after reviewer critique); refreshed **2026-07-11** to the July snapshot — see § Revision notes.
...
### The covered-class test (specified, not run)
...
selectively suppresses A. Three amendments are required before running it, all
grounded in this report's own earlier findings:
...
Feasibility: per-finding data exists (severity-tagged bullets in the upstream
`review_tasks*.md` archive) and all 78 conventions are dated by first git
appearance, but classifying ~3,300 findings against dated convention states is
new mining against the private archive — queued as a follow-up analysis rather
than run here.
