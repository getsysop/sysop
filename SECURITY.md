...
- **Bash-installer path** — tracks the HEAD of your local Sysop clone by default (`bash sysop/scripts/sysop-update.sh` copies whatever that clone is checked out at). To pin to a reviewed rev instead:
  ...
  ```
  `--ref` takes **any rev your clone can resolve**. For *pinning*, that means a release tag or a commit SHA — a branch resolves too, but a branch moves, so it pins nothing. With no usable release tag published yet (above), a commit SHA is the pin that works today. **Take that SHA from the public repo (`getsysop/sysop`), never from a mirror or tester clone.** Public `main` has appended since 2026-07-16, so a SHA taken from it after that date is resolvable — but be aware the history **was** rewritten during the first days after the 2026-07-13 launch: the repo carries three parentless root commits from 2026-07-14, -15 and -16, so any SHA predating 2026-07-16 is orphaned, including the original launch snapshot and the `v0.1.0` tag commit. Take the SHA from current `main`. Mirror and tester repos are different in kind: they are *rebuilt* — force-pushed to a fresh root commit on **every** refresh — so a SHA taken from one of those is guaranteed to stop existing at the next one, and such a repo typically carries a single commit, making the SHA you already have the only one you could pin. The rev must exist in your clone (`git -C "$SYSOP_SRC" fetch --tags` for tags), and note that the update form resolves it against **`$SYSOP_SRC`** — an unset or stale `SYSOP_SRC` resolves the pin against the wrong clone. The lock records the pinned commit, so `--check` later shows how far behind HEAD you are. Omit `--ref` to track HEAD.
