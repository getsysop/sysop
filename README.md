...
> **Prerequisites:** git, bash 4+, and Python 3 with PyYAML — Sysop's own check runner (plus, in full mode, the task validator) is a Python script, whatever your project's stack. Install PyYAML in your project's venv (`python3 -m venv .venv && .venv/bin/pip install pyyaml`) — Sysop's scripts find that venv on their own — `.venv/` or `venv/`, whether you run them from the main checkout, a subdirectory, or a worktree — so nothing needs activating; and on an externally-managed Python (PEP 668 — modern Homebrew and Debian) a bare `pip install pyyaml` refuses anyway. **macOS:** the stock `/bin/bash` is 3.2 — run `brew install bash` first (Homebrew's bash lands on your PATH ahead of the system one). **Windows:** run under WSL.
