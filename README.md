...
- **You're not working in Claude Code (yet).** The lifecycle skills are Claude Code slash commands today. The companion layer — checks, hooks, maps, workflow docs — is plain files with no Claude dependency, designed to run under any capable agent; that design is why this is a "not yet" rather than a "no". Codex users get one concrete step of it today — the installer registers the two review skills natively, so an ordinary request selects them with no prompt engineering ([§ Codex](./docs/install-and-update.md#codex)). But one step is not a track record: Claude Code is where this has actually been driven day to day, and the further you get from the review loop, the more the integration work is yours. Portability also has at least one hard edge we have measured rather than guessed — some models decline the security audit outright — so treat the two review skills as portable to different degrees, not equally. We would rather name that than sell portability we haven't tested.
...
**Where to start.** The quickstart below installs just the convention loop (`--mode loop`) — reviews that remember, rules you ratify, checks the computer runs — and leaves your branching and merge workflow alone. It's the smallest honest slice of Sysop and the cheapest to walk back, which is why it's the recommended first install for an existing codebase. (Starting from scratch? Skip the loop-first step — `/intake` in the full lifecycle is the greenfield front door: [getting-started](./docs/getting-started.md).) Before it shipped, it was run once, end-to-end, against a real ~60k-line open-source codebase — three review rounds in, a freshly mechanized convention caught an instance no round had filed. The full lifecycle is a one-flag graduation from the same install ([loop mode](./docs/loop-mode.md) is the walkthrough; [install modes](./docs/install-and-update.md#install-modes-full-and-loop) the reference).
...
The smallest install — and the recommended first one for an existing codebase — is the convention loop alone, inside whatever branching and merge workflow you already run:

```bash
git clone https://github.com/getsysop/sysop.git
bash sysop/install.sh /path/to/your/project --packs auto --mode loop
cd /path/to/your/project && git add .claude/ sysop/ CLAUDE.md .gitignore; git add .agents/ 2>/dev/null; git commit -m "chore: install Sysop (loop mode)"
```

> **Prerequisites:** git, bash 4+, and Python 3.9+ with PyYAML — Sysop's own check runner (plus, in full mode, the task validator) is a Python script, whatever your project's stack. Install PyYAML in your project's venv (`python3 -m venv .venv && .venv/bin/pip install pyyaml`) — Sysop's scripts find that venv on their own — `.venv/` or `venv/`, whether you run them from the main checkout, a subdirectory, or a worktree — so nothing needs activating; and on an externally-managed Python (PEP 668 — modern Homebrew and Debian) a bare `pip install pyyaml` refuses anyway. **macOS:** the stock `/bin/bash` is 3.2 — run `brew install bash` first (Homebrew's bash lands on your PATH ahead of the system one). **The 4+ floor is on `install.sh`** — it uses associative arrays and refuses to run below 4 (`sysop-update.sh` is a thin shim that `exec`s it, so an update run hits the same refusal); the companion scripts Sysop installs are held to **3.2**, so stock macOS runs them fine once the install itself is done. `sysop/scripts/self_check.sh` tells you which side you are on — on a 3.2 shell it says so explicitly; on 4+ it just prints the version. The stock `/usr/bin/python3` is 3.9 and is supported as-is: Sysop's scripts are tested against it on every CI run, because it is the interpreter a consumer without a venv actually runs. **Windows:** run under WSL.

`--packs auto` detects your stack (`pyproject.toml` → python, `next.config.js` → nextjs-react, and so on) and installs the matching convention packs; omit `--packs` for an interactive picker, add `--dry-run` to preview without writing. A loop install needs no plugin — the plugin's extra commands are the lifecycle skills, whose supporting scripts a loop install deliberately doesn't have. [docs/loop-mode.md](./docs/loop-mode.md) walks day one: the first review round, the first promotion, the first mechanized check.

**The full lifecycle** — planning, the task queue, worktree builds, the `/review-close` merge gate — is where the loop grows when it has earned its keep. Grow a loop install in place with `bash sysop/install.sh /path/to/your/project --update --mode full` (purely additive; nothing the loop has learned is touched), or start there directly by dropping `--mode loop` above — the commit line then gains `tasks/` (`git add .claude/ sysop/ tasks/ CLAUDE.md .gitignore; git add .agents/ 2>/dev/null`), and the commit matters: `/claim-task` builds in git worktrees, which only see committed files. Claude Code users on the full lifecycle can additionally install the slash commands as a plugin — `/plugin marketplace add getsysop/sysop`, then `/plugin install sysop@sysop`; the two paths are a pair, not alternatives (the plugin delivers only the slash commands — everything they operate on arrives via the bash installer). Updating, pinning to a release, plugin mechanics, permissions: [docs/install-and-update.md](./docs/install-and-update.md).

## Documentation

- **Start here** — [`docs/loop-mode.md`](./docs/loop-mode.md): day one with the recommended `--mode loop` install — run the convention loop against an existing repo, keeping your own branching and merge workflow.
- **Go full lifecycle** — [`docs/getting-started.md`](./docs/getting-started.md): a hands-on walkthrough from install to your first shipped change (install → `/intake` → `/claim-task` → `/review-close`).
...
- **Un-claim a task** (full lifecycle): `bash sysop/scripts/claim_task.sh --release <TASK-ID>` (from the main checkout) removes the task's worktree — never the main one — flips its `status:` back to `open`, deletes the lock, and prints the commit to run. `--force` discards uncommitted worktree work; `--delete-branch` drops the feature branch too.
- **Un-claim a review batch:** `bash sysop/scripts/batch_work.sh --release <BATCH-NUMBER>` (also from the main checkout) is the batch-side twin — a batch holds a real lock too. It removes the worktree, reverts the batch header and its checkboxes in `review_tasks.md`, and releases the lock. `--force` is required if any task in the range is already checked off.
- **Remove Sysop entirely:** revert the install commit if nothing has touched the payload since; otherwise `rm -rf sysop/` (everything Sysop owns lives under one dir), then revert the `.claude/` merge, the two `.agents/skills/` symlinks (delete the *links* only — never `.agents/skills/` recursively, since it may hold entries of your own), and the `CLAUDE.md` / `.gitignore` appends. Because the vendor files live under `sysop/` and never mingle with your own, there's nothing to pick apart. Then disarm the hooks — the one untracked surface: `rm -f .git/hooks/pre-commit .git/hooks/pre-merge-commit` (any hooks of your own that the installer displaced were backed up alongside as `*.bak.<timestamp>`). Plugin users: also `/plugin uninstall <plugin>@sysop`.
