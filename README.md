...
cd /path/to/your/project && git add .claude/ .agents/ sysop/ CLAUDE.md .gitignore && git commit -m "chore: install Sysop (loop mode)"
```
...
**The full lifecycle** — planning, the task queue, worktree builds, the `/review-close` merge gate — is where the loop grows when it has earned its keep. Grow a loop install in place with `bash sysop/install.sh /path/to/your/project --update --mode full` (purely additive; nothing the loop has learned is touched), or start there directly by dropping `--mode loop` above — the commit line then gains `tasks/` (`git add .claude/ .agents/ sysop/ tasks/ CLAUDE.md .gitignore`), and the commit matters: `/claim-task` builds in git worktrees, which only see committed files. Claude Code users on the full lifecycle can additionally install the slash commands as a plugin — `/plugin marketplace add getsysop/sysop`, then `/plugin install sysop@sysop`; the two paths are a pair, not alternatives (the plugin delivers only the slash commands — everything they operate on arrives via the bash installer). Updating, pinning to a release, plugin mechanics, permissions: [docs/install-and-update.md](./docs/install-and-update.md).
