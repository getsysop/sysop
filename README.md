...
> **Prerequisites:** git, bash 4+, and Python 3 with PyYAML — Sysop's own check runner and task validator are Python scripts, whatever your project's stack. Install PyYAML in your project's venv (`python3 -m venv .venv && .venv/bin/pip install pyyaml`) — Sysop's scripts resolve the repo's `.venv` on their own, and on an externally-managed Python (PEP 668 — modern Homebrew and Debian) a bare `pip install pyyaml` refuses anyway. **macOS:** the stock `/bin/bash` is 3.2 — run `brew install bash` first (Homebrew's bash lands on your PATH ahead of the system one). **Windows:** run under WSL.
