...
>
> **Glob patterns** like `<api module>/server.py` name your real paths only once you localize them. Editing them into the assembled `.claude/convention_map.md` *alone* is erased — that file is regenerated on every update. Localized sections are therefore **dual-written** (`_shared/promotion-write-target.md`): into the assembled `.claude/convention_map.md`, which is what the current review round reads, **and** into the never-managed `.claude/convention_map.project.md` overlay, which is what survives the next update and re-supplies the section. Overlay-only leaves the section inert until that update runs; base-only is erased by it. `/codebase-review` writes both when its coverage sweep proposes a section. (Full explanation: `docs/packs.md` in the Sysop repo.)
>
