...
Claim the task with `/claim-task <TASK-ID>`. The claim itself flips `status: open → in_progress` in `tasks/index.yml` and creates `sysop/runtime/locks/<TASK-ID>.lock` (the validator's `in_progress` invariant requires both) — but the claim is where the command *starts*, not where it stops; see the orchestrator note below.
...
**`/claim-task` does not stop at the claim any more.** Since Phase 171 it is an *orchestrator*: after the lock and worktree it spawns a planner, spawns an independent reviewer of that plan, classifies the findings itself, optionally gates on you, and spawns an executor — writing `plan.md`, `review.md` and `classification.md` under `sysop/runtime/claim/<CLAIM_ID>/<RUN_ID>/` as it goes. It asks up front how much you want to be in the loop (`## Plan review`, or the `--review-plan` / `--no-review-plan` / `--plan-only` flags), and `--plan-only` stops after the review, writes the reviewed plan into the task body and releases the claim. `/review-close` and `/sitrep` both *report* on the artifacts it leaves; neither blocks on them. `WORKFLOW.md` § 2.2 is the authoritative description. **Steps 3 through 5 below are the manual path** — what to do when you are working the task yourself rather than handing it to `/claim-task`, and what the sub-agents are doing on your behalf when you are not. They are not a second set of steps to run after it.
...
branch_tip: 4f2a9c1e8b7d6053a1c4e9f27b830d5a6c1e4f92
date: 2026-03-20
type: feature
...
**Fix it in the branch before you file it.** When the work turns up something adjacent, take three tiers in order. Fix it here when *all* of these hold: it is in a file the task already touches; it is mechanical, or a doc, test or convention-config correction; a gate covers it or you add the test that does; it is small — around 20 lines, a few per branch; and it is not a claim about what the code *means* that you have not verified by reading the consumer. Never, at any size: migrations, prompts under an eval gate, auth and money-path code, the project's security-critical always-include files, production writes, and **anything that would weaken or disarm a gate** — that last one because tier 1's "a gate covers it" test is satisfied by the disarming change itself. Otherwise extend an existing open task in that module; otherwise file a new one — and filing has a bar of its own: **name what the new task blocks**, whether that is the phase you are currently in, a later phase you have actually planned, a gate your project declares, or an open task this stops. A design question, anything needing your hands, a production write, and any security finding are filed regardless — as is a defect in shipped behaviour, provided you can state the failure: the input, what you expected, what actually happens. If you cannot state it that concretely you have a suspicion rather than a defect, and a suspicion is what the ledger is for. What is left over — real, but nothing is waiting on it — goes as one line to `tasks/notes.md`, a flat ledger beside the queue that nothing routes to and nothing counts. `/add-task` reads it, so promoting a note later costs nothing; leaving it there costs nothing either, which is the difference between a note and a task. Record each in-branch fix as one line under `## Also fixed` in the task body, so the close reads it as intended scope rather than as an unexplained hunk.
...
- **`### Ratchet (changed files only)`** — a bash code block of project-supplied snippets. Each snippet pipes `git diff --name-only "$BASE_REF"...HEAD` through a file-type filter and invokes lint or typecheck against only the changed files, where `$BASE_REF` is **resolved** from `default_branch.sh` rather than hard-coded to `origin/main` (`Q-380` — on a `master`-default repo the literal names no ref, the filter comes back empty, and an empty filter *passes*, so the ratchet reports green over the whole branch). Snippets short-circuit and pass when no changed file matches the filter — which is why *which tree they run on* decides whether they check anything: on the pre-merge pass that range is the default branch's local-only commits (often empty), and only on the merge target does it name the work.
...
- **`/roadmap`** — reads **both queues** back at portfolio level: where the project stands against `tasks/vision.md`, the outstanding work grouped by kind (feature / data-ops / infra, plus review batches from `review_tasks.md`) with readiness flags, and up to four proposed orderings of attack (unblock-the-human-first / foundation-first / ship-fast / drain-the-review-queue) with the trade-off behind each — each closing with a copy-pasteable `Run it:` actuator line (`/auto-build <IDs>` / `/claim-task <ID>`, or `/auto-fix --batches <N>` / `/auto-judge --batches <N>` for a batch) so an ordering is a handoff, not just advice. The judgment sibling of `/sitrep` — `/sitrep` classifies what's *in flight now*; `/roadmap` strategizes what's *left*. Read-only; the demo beat between `/intake` (populate the queue) and `/auto-build` (execute it).
