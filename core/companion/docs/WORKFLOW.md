...
6. **Close merged batches** — **determine the batch set first**: the review batches whose branches step 5 merged, read from `review_tasks.md` (via `review_index.py --list`, which is the authoritative list of batches, their status and their branches) and corroborated against any `BATCH-*.lock`. **Not from the pending-doc's `review_task_ids`** — those are `TASK-NNNN` ids and this script takes batch numbers. An **empty** set is the ordinary single-task cycle: report it and skip the step, including its landing gate, which looks for a commit a zero-batch cycle never produces. Then run `bash sysop/scripts/close_batch.sh <N1> <N2> ...` on the merge target to flip batch headers to `Merged`, check off task boxes (except any annotated `> Failed:`, which stay open), and update the Statistics table (under `pr` policy the gate resolves against the checked-out merge target, so the **integration-branch** shape needs no `--force`; the **PR-reuse** shape still does, because there HEAD *is* the approved branch and no ancestry evidence exists until the PR squashes). This runs after merges but before doc consolidation
