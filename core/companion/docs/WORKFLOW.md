...
**YAML-driven selection:** `tasks/index.yml` is the source of truth for task metadata. `scripts/validate_tasks.py` enforces schema invariants on every commit, so `/next-task` filters the index directly — no prose-format fallback rubric. Selection rules:

- Anchor on the phase with `current_focus: true` (validator guarantees exactly one).
- Keep tasks with `status: open`, `user_action: false`, no active `.locks/<id>.lock`, and all `depends_on:` IDs resolved to `status: done`.
...
**Inputs:** `tasks/index.yml`, `tasks/<status>/<TASK-ID>.md`, `review_tasks.md`, `.locks/*.lock`
...
3. **Claim the task** — create an isolated git worktree on a feature branch (via `scripts/claim_task.sh`). `/claim-task` Step 4 always passes `--lock` so the validator's `in_progress requires lock` invariant is satisfied; the lock lives under `<main-repo-root>/.locks/<TASK_ID>.lock` regardless of which worktree invokes the script (Phase 32). Solo workflows that bypass `/claim-task` and call the script directly may omit `--lock`
...
**Outputs:** Git worktree, feature branch, implementation plan, single commit in the worktree (made by the reviewer-executor sub-agent), envelope-reported handoff plus sealed `REVIEW_REPORT:` (plus an optional `.locks/<TASK_ID>.lock` file if `--lock` was used)
...
2. **Apply batch-sizing rule** — score each candidate by `effort_weight × blast_radius_weight`, enforce architectural-solo and migrations-solo invariants, cap cross-module tasks at one per batch, cap total weight at K=6 and raw count at N=4
...
5. **Per-task three-phase orchestration** (parallel across tasks, sequential within a task):
   ...
   - **Phase 6d** — halt-on-blocker (park the task; write `plan.md` + `review.md` to `<worktree>/.auto-build/` for the human to resume) OR pass `PLAN_TEXT + RAW_FINDINGS` to the execution agent
...
6. **Phase 7 envelope recovery** — for tasks whose execution agent omitted the YAML envelope, cross-check the filesystem (commit landed AND `.pending-docs/<branch>.md` exists); if both hold, reclassify as `EXECUTED (envelope-recovered)` so the failure mode is visible without false-FAILing real work
...
**Inputs:** `tasks/index.yml` (current-focus phase + open tasks), `.claude/convention_map.md`, `.claude/security_map.md`, `<project>/CLAUDE.md § Pre-merge verification`, optional `.locks/*.lock` for concurrent-claim detection
**Outputs:** Up to N worktrees on feature branches, per-task commits inside each worktree, per-task `.pending-docs/<branch>.md`, status table with envelope details
**Checks:** Batch-sizing rule (effort × blast_radius weights, K=6 sum ceiling, N=4 count cap), DB-contention warning, post-Phase-6a integrity check (plan-only agent didn't commit), post-Phase-6e envelope parse, Phase 7 filesystem cross-check
...
4. **Run deterministic pre-scan** (`bash scripts/run_checks.sh`) — five stages inside one invocation, all sharing the same `(check_id, file_line, message)` finding shape so baseline matching, `--mode` filtering, and `--fail-on-blocking` apply uniformly:
   ...
   - **(e) pip-audit** — `pip-audit --strict --format json` against the active venv. Single catch-all check ID `pip-audit-vuln` anchored at the first discoverable `requirements*.txt` (sorted); each finding embeds package name, version, CVE/GHSA ID, fix version, and description. Findings tagged `[pip-audit]`. Security-audit-only (codebase-review skips A06). Silently skips if `pip-audit` is missing. **Known gap:** audits only the venv that pip-audit runs in — projects with multiple service-specific venvs need a per-venv invocation
...
8. **Extract convention candidates** — scan findings for recurring patterns (3+ occurrences across different files). Draft candidate prevention convention bullets, attribute each to this review round, and write to `.pending-docs/convention-candidates.md`. The 3+ burst is the in-round *noticing* signal; promotion (step 9) gates on cross-round survival — see § 3.5
...
3. **Run verification** — full CI gate (test suite, production build, any project-specific checks). After verification, the new Step 3c (Phase 35, BeanRider ISSUE-0008) gates on `manual_smoke: true` tasks and `.pending-docs/` smoke headings: if any merged branch has a documented manual smoke procedure, the skill halts and prompts the human via `AskUserQuestion` to drive the smoke, confirm they ran it, or waive it (logged in the final report)
4. **Prepare worktrees for merge** — for each approved feature branch, copy any `.pending-docs/*.md` from the worktree back to main's `.pending-docs/` (they're untracked and would otherwise be lost), then remove the worktree so the branch can be checked out on main
...
10. **Clean up** — `direct`: delete merged branches (local + remote), remove `.pending-docs/`. `pr`: only after the PR confirmed `MERGED` — re-sync local `main` to `origin/main`, drop the integration branch and the squash-merged feature branches; if the PR is stuck, skip cleanup so the work stays recoverable
...
**Inputs:** Feature branches, `.pending-docs/*.md`
...
**What happens:**
1. Remove merged worktrees: `scripts/cleanup_worktrees.sh --clean`
...
**Inputs:** Git worktree state, `.locks/`, `review_tasks.md`
**Outputs:** Clean filesystem and git state

**Implementing scripts:** `scripts/cleanup_worktrees.sh`, `scripts/close_batch.sh`, `scripts/archive_review_tasks.py`
...
4. Write candidates to `.pending-docs/convention-candidates.md`, each **attributed to the current review round** (the attribution is what makes the cross-round gate below computable)
...
2. For promoted candidates:
   a. **Choose enforcement mechanism (mechanical-first).** Walk this menu in order; stop at the first option that fits:
      ...
      - **(iii) `scripts/hooks/pre-commit` regex** — same regex shape as (i) but fires in the editor cycle. Choose (iii) when local feedback matters; (i) when CI is the canonical gate; both is acceptable.
...
**Lock file coordination** (for parallel agent work) is an optional extension not covered here. It uses YAML lock files in `.locks/` to track which files each agent is modifying, enabling conflict detection at claim time. Lock files use `claim_task.sh --lock` to create `.locks/<TASK_ID>.lock` files with task ownership metadata. Locks live under the **main** repo root (resolved via `git rev-parse --git-common-dir`) regardless of which worktree the script was invoked from; the validator (`scripts/validate_tasks.py`) uses the same resolution so every caller — pre-commit hooks inside worktrees, `/review-close` from main, ad-hoc CLI invocations — agrees on one canonical lock state (Phase 32, 2026-05-22 — closes BeanRider ISSUE-0028 / 0030 / 0032 / 0013).
...
**Scaling:** When the tracker exceeds **125KB**, archive merged rounds to a separate file (`review_tasks_archive.md`) using `scripts/archive_review_tasks.py`. `/auto-fix` and `/auto-judge` enforce this threshold and refuse to run above it; below it, archival is a manual housekeeping step. Not needed until you have 50+ batches.

**Shadow JSON index:** Scripts that consume `review_tasks.md` read from a shadow JSON index (`.claude/review_index.json`) instead of parsing Markdown directly. The index is auto-generated by `scripts/review_index.py` and auto-rebuilds when the Markdown changes (checksum-based drift detection). The Markdown remains the human-readable source of truth; the JSON is the machine-readable interface. Scripts (`batch_work.sh`, `close_batch.sh`) fall back to inline regex parsing if Python is unavailable. After any Markdown mutation, the mutating script rebuilds the index.
...
- Installed via `scripts/install_hooks.sh`
- Must be re-installed after creating a new worktree

**Pre-merge hook** (`scripts/hooks/pre-merge-commit`):
...
**Installation:** Run `bash scripts/install_hooks.sh` after cloning the repo or creating a worktree. The hooks are tracked in `scripts/hooks/` and copied to `.git/hooks/` by the install script.
...
Review finds pattern (3+ occurrences in a round)
  → Convention candidate recorded, attributed to that round (.pending-docs/convention-candidates.md)
...
`scripts/ci/sysop-checks.yml.example` (delivered but **unarmed** — like the git-hook
templates). It runs the Sysop blocking-findings gate (`run_checks.sh --fail-on-blocking`)
plus your own test suite. To enable it:

1. `cp scripts/ci/sysop-checks.yml.example .github/workflows/sysop-checks.yml`
...
**Skeleton** (`scripts/hooks/pre-commit` — for a new project):
...
Deterministic checks detect convention violations without LLM interpretation. The shared registry at `.claude/checks.yml` defines grep entries directly; LSP (`pyright-*`, `tsc-*`), Semgrep (`semgrep-*`), and coverage (`coverage-*`) check IDs are auto-populated at scan time by `scripts/run_checks_impl.py`. All sources share the same `(check_id, file_line, message)` output shape, so baseline matching, `--mode` filtering, and `--fail-on-blocking` apply uniformly. Both `/codebase-review` and `/security-audit` consume the same registry so every check has exactly one definition.

Run the registry directly with:

```bash
bash scripts/run_checks.sh                         # All checks (advisory)
...
| `paths` | Yes | Directories to search |
| `include` | Yes | File glob patterns |
| `exclude` | No | File glob patterns to skip |
...
**LSP / typechecker findings.** `run_checks_impl.py` invokes `pyright` (Python) and `tsc --noEmit` (TypeScript) after the grep loop. Each diagnostic becomes a synthetic check record with a `pyright-*` or `tsc-*` ID. These do not appear in `checks.yml` — they are auto-generated from the typechecker output. If a binary is missing, the relevant half silently skips with a stderr warning. `pyright-general-warning` is a catch-all; treat it the same as grep checks with `notes: "Needs LLM triage"`.
...
**pip-audit findings.** `_run_pip_audit()` invokes `pip-audit --strict --format json` against the active venv. All vulnerabilities map to a single catch-all `pip-audit-vuln` check_id (registered in `checks.yml`, `used_by: [security-audit]`). Findings are anchored at the first discoverable `requirements*.txt` (sorted via `_find_requirements_files()`) since pip-audit reports per-package, not per-line; the package name, CVE/GHSA ID, and fix version are embedded in the message. Falls back to `requirements.txt:1` when no requirements file is present. Skips silently if `pip-audit` is missing. **Known gap:** the audit only sees the venv pip-audit runs in; projects with multiple service-specific venvs need a per-venv invocation.
...
**Semgrep AST rules.** Rules live in `.claude/semgrep/<rule-name>.yaml` (not in `checks.yml`). Each rule's `id:` field becomes the check ID prefixed with `semgrep-` (so `id: sql_fstring` → `semgrep-sql_fstring`). Fixtures for each rule live in `.claude/semgrep/fixtures/`. See `.claude/semgrep/README.md` for authoring conventions. The entire stage silently skips if `semgrep` or `.claude/semgrep/` is absent.
...
**Deferred docs schema** (`.pending-docs/<sanitized-branch-name>.md`):
...
- [ ] Create `scripts/hooks/pre-commit` with the skeleton from § 6.4 (no checks yet — add them as conventions accumulate)
- [ ] Create `scripts/install_hooks.sh` to copy hooks to `.git/hooks/`
- [ ] Copy this `WORKFLOW.md` and `WORKFLOW_GUIDE.md` into the project root
- [ ] Port the lifecycle skills from § 8.3 (`/next-task`, `/claim-task`, `/document-work` cover the core loop; add others as needed)
- [ ] Port `scripts/claim_task.sh` and `scripts/cleanup_worktrees.sh` from the reference implementation (§ 8.4)
...
| `.claude/review_index.json` | auto-generated | Shadow JSON index of `review_tasks.md`; rebuilt by `scripts/review_index.py` |
...
| `Bash(bash scripts/claim_task.sh:*)` | `/claim-task`, `/auto-build` (Step 5.2 sequential pre-claim, one invocation per batched task) |
| `Bash(bash scripts/batch_work.sh:*)` | `/auto-fix`, `/auto-judge` |
| `Bash(bash scripts/close_batch.sh:*)` | `/review-close` |
| `Bash(bash scripts/run_checks.sh)`, `Bash(bash scripts/run_checks.sh:*)` | `/codebase-review`, `/security-audit`, `/review-close` |
| `Bash(bash scripts/install_hooks.sh)` | _(direct user invocation only — `claim_task.sh` shells out to it, but that's a nested subshell that doesn't go through the classifier)_ |
| `Bash(python scripts/archive_review_tasks.py:*)`, `Bash(python3 scripts/archive_review_tasks.py:*)` | `/codebase-review`, `/auto-fix`, `/auto-judge` (when `review_tasks.md` exceeds 125KB) |
| `Bash(python3 -c:*)`, `Bash(python3 -:*)` | `/claim-task` Step 2/4, `/auto-build` Step 2 (batch-sizing math heredoc) + Step 5.1 (yaml-round-trip status flip), `/review-close` Step 4c (heredoc'd yaml.safe_load / safe_dump round-trips against `tasks/index.yml`), `/document-work` Step 3b (heredoc'd follow-up-stub check against `tasks/index.yml`) |
| `Bash(python3 scripts/validate_tasks.py)`, `Bash(python3 scripts/validate_tasks.py:*)` | `/auto-build` Step 5.3 (post-claim schema validation), `/review-close` Step 4c (final-guard task-state validation) |
...
**Recommended invocation.** From the consumer project root: `bash scripts/sysop-update.sh`. The shim resolves the consumer root via `git rev-parse --show-toplevel` and the Sysop source via `$SYSOP_SRC` (an env var the consumer sets once in their shell rc — absolute path, no `~`), then exec's `bash "$SYSOP_SRC/install.sh" "<consumer-root>" --update "$@"`. All flags forward verbatim (`--force`, `--packs`, `--dry-run`, `--yes`, `--no-arm-hooks`, `--ref <tag>`), and the install.sh exit code propagates. The shim is shipped to `<target>/scripts/sysop-update.sh` by the bash installer and refreshed on every `--update` run (it is a managed path), so improvements to the wrapper itself flow through the standard update channel.
...
`managed_paths` is the explicit list of every path the installer wrote (workflow docs, `.claude/`, `scripts/`, `scripts/hooks/`, `scripts/ci/`). It's the contract that future `--update` and `--check` runs honour — files outside this list are user-owned and never touched.
...
10. **Phase 15 armed-hook divergence check** (always runs, all modes): compare each `scripts/hooks/<base>` against `.git/hooks/<base>` (the actually-armed copy, untracked by git). Differences mean the armed hook is stale — either `--update` skipped auto-arm and the consumer has not yet re-armed, or `.git/hooks/` was hand-edited out of band. Read-only signal; never modifies hooks. The fix is `bash scripts/install_hooks.sh` after the consumer settles on the desired `scripts/hooks/<base>` body.
...
**Armed-hook safety (Phase 15 / BeanRider ISSUE-0007).** `.git/hooks/` is not tracked by git — `git diff` cannot reveal staleness there. Surfaced during BeanRider's first `sysop-update.sh` run (Phase 14's first real-world absorption), where pre-Phase-15 `install.sh` overwrote `scripts/hooks/pre-commit` with the upstream skeleton, then armed the skeleton into `.git/hooks/pre-commit` before the consumer reconciled — silently swapping a working bean-check guard for a no-op for the duration of the reconcile window. Phase 15 splits the contract: `--update` copies `scripts/hooks/*` (so the consumer can reconcile) but does NOT arm; the consumer re-runs `bash scripts/install_hooks.sh` after reconciling. The always-run armed-hook divergence check (step 10) catches not just the reconcile-window race but any case where `.git/hooks/<base>` falls out of sync with `scripts/hooks/<base>` — fresh-install hand-edits included. Fresh installs continue to auto-arm (no race window when there is nothing to reconcile). **Phase 24 update (see § 8.2c below):** the `scripts/hooks/*` *template* bodies themselves are now also preserved when the consumer has modified them since the last absorption — closing the gap where the pipeline used to overwrite a consumer-customized template with the upstream skeleton. The Phase 15 protection still applies (auto-arm is skipped in `--update`); Phase 24b adds the template-body preservation on top.
...
**pyyaml dependency.** The id-collision pre-scan and the collision-case merge both need pyyaml. `install.sh` picks the python3 to use via `pick_python_with_yaml()` — tries `<target>/.venv/bin/python3`, then `<target>/venv/bin/python3`, then `python3` on PATH. If none can `import yaml` AND a `.claude/checks.project.yml` exists, the install aborts with an actionable error pointing at `python3 -m venv .venv && .venv/bin/pip install pyyaml`. The markdown suffix files have no pyyaml dependency. Worked example in [README.md "Updating an existing install"](../../../README.md).
...
After `bash scripts/sysop-update.sh`, every `paths: ["<api module>/", "<scripts dir>/", "<datajobs dir>/"]` in the upstream `.claude/checks.yml` becomes `paths: ["parsers/", "scripts/", "streamlit_app/"]`. Checks now fire against real directories.

### 8.2c (continued) — Phase 24b: ancestor-aware preservation of consumer-modified scripts/* and scripts/hooks/*
...
1. **First run:** `bash scripts/sysop-update.sh` (or `... --dry-run` for a plan-only pass). Output reports preserved paths and `preserved: N managed path(s) preserved due to consumer modification`.
2. **Agent inspects** each preserved path's pending Sysop-side change against the consumer-side current content: `git -C "$SYSOP_SRC" log --oneline -p "<old_commit>..HEAD" -- core/companion/scripts/<file>` and `git -C "$TARGET" log --oneline -p -- scripts/<file>`. Decides per-file: take upstream, keep consumer, or merge by hand.
3. **Second run** (only if any preservation should be overridden): `bash scripts/sysop-update.sh --accept-upstream-list /tmp/accept.txt` (one path per line) — or `--accept-upstream <path>` flags for small sets.
...
| `install_hooks.sh` | Copy `scripts/hooks/*` into `.git/hooks/` (re-run after new worktree; required after `--update` reconciliation per Phase 15 / ISSUE-0007 — `install.sh --update` no longer auto-arms) |
...
|------|-------------|---------|
| `.locks/<TASK_ID>.lock` | `claim_task.sh --lock` | Optional multi-agent coordination |
| `.pending-docs/<branch>.md` | `/document-work` | Deferred documentation, consolidated at merge |
| `.subagent-envelopes/<TASK_ID>.json` | `parse_subagent_envelope.py` (SubagentStop hook) | Structured envelope receipt from `/claim-task` reviewer-executor + `/auto-build` execution agents (Phase 37). Consumed and deleted by the parent skill; persists only on hook-parse miss as `_unparseable_<session>_<agent>.json` for diagnostics. |
| `.auto-build/parked/<TASK_ID>__<timestamp>.md` | `/auto-build` Phase 6d | Durable archive of a parked task's plan + adversarial verdict, mirrored from the per-worktree `.auto-build/` scratch so it survives `cleanup_worktrees.sh --force` (Phase 65a). Lives at the project root, not in the worktree. |
...
7. Port scripts from `scripts/` (adapt paths, test commands, frontend directory name)
8. Install hooks: `bash scripts/install_hooks.sh`
9. Create `review_tasks.md` with one empty `## Round 1` section
10. Run `bash scripts/run_checks.sh` to verify the check registry loads (should report zero findings on the empty registry)
