...
4. **Isolation by default.** Work happens in git worktrees on feature branches, so no two builds collide in the tree. The main branch is not a *build* surface — but it is not untouched either: claims, closes and review rounds commit their small coordination writes there (`tasks/index.yml`, `review_tasks.md`), each under an explicit `HEAD` assert. What you should never do by hand is **freehand work** on it (§ 4.4). Merges go through a verification gate.
...
4. **Resolve the plan-review preference** (Step 6) — before any agent spawns, decide how much the human is in the loop: option **A** (human gate between review and implementation), option **B** (run it unattended), or option **C** (plan only — stop after review, write the reviewed plan into the task body, release the claim). Resolution order is flag → `<project>/CLAUDE.md § Plan review` → ask, per `_shared/plan-review-preference.md`. Front-loaded on purpose: planning plus review is 5–25 minutes, so asking afterwards asks someone who may have walked away. **Option C is roadmap-only** — a review batch has no body file to persist a plan into, so a batch is offered two options, not three, and `--plan-only` is rejected at Step 1 before the batch is claimed
...
10. **Spawn an executor** (Step 7e) — reads the plan, review and classification from disk, implements, applies the **fix-in-branch tier** to anything adjacent it surfaces (Sequence item 2b: fix it in this branch under the stated bound and never-list, or extend an open task in that module, or file — in that order, recording each in-branch fix under `## Also fixed`; a filing that reaches tier 3 must name what it blocks — the `current_focus` phase, a named `planned` phase, a gate the project declares, or an open task's stated acceptance — and otherwise goes to the `tasks/notes.md` ledger instead of the queue, the exemptions being a design question, a `user_action`, a production write, and a defect in shipped behaviour statable as a falsifiable failure, or a security finding), persists the `## Test decision` **into the worktree's copy of the task body** so it rides the branch (an edit made in the main checkout is committed by nothing and reaches no PR — internal tracker #322), runs post-fix convention verification, runs the consumer's `## Pre-merge verification` gates *in the worktree*, runs UI verify if frontend files changed, and makes a single commit carrying a `Doc-Work:` trailer. It does not push and does not invoke `/document-work`
11. **Receive the executor envelope and hand off** (Step 8) — read `sysop/runtime/subagent-envelopes/<CLAIM_ID>.exec.json`, resolved against the main repo root. On `EXECUTED`, print the sealed report — saying so explicitly if `review_report_raw` was null rather than printing nothing — run the stranded-body check (tracked modifications under `tasks/` in the main checkout, which reach no PR), **read the `## Test decision` back off the branch tip** (Phase 249 — a fence-aware probe run by the orchestrator, not by the executor that was supposed to write it, because the failure being closed is an executor skipping a sequence item; `MISSING` and `TEMPLATE` both **halt before `/document-work`**, while `NOT ON BRANCH` and `UNREADABLE` report and continue, since neither asserts anything about the record), and — only if that passed — direct the user to `/document-work`. On `BLOCKED`, park. On `FAILED` or a malformed envelope, surface the error verbatim and let the human decide — no auto-retry
12. **Write the plan back and release** (Step 7f, option C only) — instead of gating or executing, write the reviewed plan and its `## Test decision` into the **main checkout's** body file, commit that on `main` under `_shared/main-push-guard.md` Rule A, run `claim_task.sh --release`, and commit the release flip. Ordered so the durable product lands first: `--release` refuses in five ways, and an earlier ordering that released before committing lost the plan on every failure path. The body gains a `## Plan` section holding the plan verbatim plus the sealed `REVIEW_REPORT:` that passed it — on the one path with no implementation, the review *is* the deliverable. A later `/claim-task` on that task finds the section, **skips the planner and re-reviews the plan** against a `main` that has moved. Option C never reaches Step 8: there is no executor, so there is no envelope and no `/document-work` handoff
...
**What the shape does and does not fix, stated plainly.** It removes the `ExitPlanMode` attractor and it makes a skipped review *visible*, because the orchestrator's output is now an artifact set that a blocked run cannot produce. It does **not** prevent a harness that forbids the `Agent` tool outright — a system-prompt instruction outranks skill text — and Sysop reports that case rather than stopping it. **All three of part B's legs now ship, and what they buy is reporting rather than prevention**: `/review-close` Step 2e prints the per-branch artifact set (leg 1, Phase 237), `/sitrep` classifies a parked or awaiting-approval claim as such instead of as `planning` (leg 2, Phase 237), and close-time cleanup removes the set once the close lands on `main` (leg 3, Phase 236 — `/review-close` Step 4c for a roadmap claim, `close_batch.sh`'s `remove_claim_artifacts()` for a batch). **Both readers report and neither rejects**, deliberately: a reporting reader that mis-resolves prints "artifacts: none found" beside a branch that has them — wrong, visible and harmless — while a rejecting one false-FAILs work already done, which is how the Phase-155 attempt died. So a blocked run is now conspicuous in two places a human already looks; it is still not impossible, and describing either reader as a gate overstates what shipped. One limit this paragraph used to state is gone: until Phase 248 `/sitrep`'s predicate was reached on the **roadmap** path only, because `run_survey`'s lock loop skips a `BATCH-`/`TASK-` prefixed id before the classifier runs; `_classify_review_batches` now makes its own call (`Q-317`), and since Phase 252 both paths read the park evidence beside the commit count instead of gating the probe on a claim having no commits (`Q-362`) — a claim that parked after producing commits reports `parked, work in progress`, and one that resumed after a park reports its ordinary state with the marker noted. Of the four artifacts, only the `SubagentStop`-written envelope is unforgeable: `plan.md`, `review.md` and `classification.md` are ordinary writes at paths the orchestrator also knows. `BLOCKER_QUESTION:` vs `PARKED_REASON:` still differs from `/auto-build` and is still load-bearing — see `_shared/adversarial-review.md`.
...
---
branch: <branch-name>
branch_tip: <full SHA — git rev-parse HEAD, stamped LAST>
date: <YYYY-MM-DD>
...
2. **Apply batch-sizing rule** — score each candidate by `effort_weight × blast_radius_weight`, enforce the architectural-solo, migrations-solo and declared-`solo:` invariants, cap cross-module tasks at two per batch, cap total weight at K=12 and raw count at N=4
...
4. **Sequential pre-claim on `main`** — for each task, flip `status: open → in_progress` in `tasks/index.yml`, create the worktree + lock via `claim_task.sh --lock`, validate schema, commit the claim via `claim_task.sh --commit-claim` (message shape `claim: mark <TASK_ID> as in-progress`)
...
2. **Review** — for each branch:
   ...
   - Verify any **`## Also fixed` record** (Step 2d's `2‑also` arm, Phase 276): when a branch's task body carries the section, check that every path a line names is in the branch diff, that no line names a never-tier-1 path, and that the count is small. Absence is not a finding — the section is present only when a branch carried an adjacent fix.
   - Verify the recorded **test decision** (Step 2d, Phase 59): read the `## Test decision` section the **executor** wrote into each approved branch's task body, in the worktree, during implementation (the planner *decides* it at plan time and emits the step that writes it — Phase 58b) and check it against the diff — "plan said test X — is it here?" / "plan said no-test-because-Z — does Z still hold?". On a mismatch or a missing record, halt and prompt via `AskUserQuestion` with **four** dispositions: accept the record, hold the branch for a fix (demotes to rejected this run), waive, or — Phase 249 — *record not owed*, which is offered **only** when the ownership probe finds no claim lock and only on a `missing` record. A lock present means the record *was* owed, and the escape is withheld, so the fourth disposition cannot become a quieter waiver for the misses the Step 8 read-back exists to surface. `waived` and `not owed` are tallied separately; summing them restores the conflation the disposition was added to remove. Verify the record, don't re-judge whether a test should exist (that is the plan-time adversarial reviewer's job — `_shared/adversarial-review.md` finding 7)
   - **Report the claim artifacts** (Step 2e, Phase 237): for every branch under review, print which of `plan.md`, `planner-integrity.md`, `review.md`, `classification.md` and `outcome.md` exist for that branch's claim, plus any hook envelopes, and the recorded verdict. Resolve the branch to a claim id from the **lock**, never from the branch name — `--branch` and the index's `branch:` field both override the generated name. **It never changes a disposition**: a rejecting form of this check was built in Phase 155 and disqualified by its own round (inert on the batch claims it was reported for, false-FAIL on the dominant path). Absence renders as **unknown**, never as "did not run" — `sysop/runtime/` is gitignored and the `SubagentStop` hook is Claude Code only, so a resume, a fresh clone and a non-Claude harness all legitimately carry nothing
...
4. **Prepare worktrees for merge** — for each approved feature branch, collect any `sysop/runtime/pending-docs/*.md` from the worktree into main's `sysop/runtime/pending-docs/` (they're untracked and would otherwise be lost), then remove the worktree so the branch can be checked out on main. **The collect is provenance-checked, not an unconditional copy:** a basename is not unique to a branch, so if main already holds a doc of that name authored by a *different* branch, the step refuses — nothing is collected, main is untouched, the worktree stays and the branch is SKIP'd for this run. Same-branch means main's copy is a stale twin and the worktree wins. **The claim's artifact set is untouched by this removal and that is by design** — `sysop/runtime/claim/<CLAIM_ID>/<RUN_ID>/` lives in the *main* checkout, resolved via `git rev-parse --git-common-dir`, precisely so the whole per-run set — the plan, the planner's integrity record, the review, the classification, the outcome, and option C's release marker — outlives `git worktree remove`; step 7 is what removes them, once the close has landed
...
6. **Close merged batches** — **determine the batch set first**: the review batches whose branches step 5 merged, read from `review_tasks.md` (via `review_index.py --list`, which is the authoritative list of batches, their status and their branches) and corroborated against any `BATCH-*.lock`. **Not from the pending-doc's `review_task_ids`** — those are `TASK-NNNN` ids and this script takes batch numbers. An **empty** set is the ordinary single-task cycle: report it and skip the step, including its landing gate, which looks for a commit a zero-batch cycle never produces. Then run `bash sysop/scripts/close_batch.sh --merge-target <merge target> <N1> <N2> ...` **on the merge target** — the script commits `review_tasks.md` to whatever branch is checked out, so where you stand still decides where the record lands even though it no longer decides what the gate verifies — to flip batch headers to `Merged`, check off task boxes (except any annotated `> Failed:`, which stay open), and update the Statistics table (under `pr` policy the gate resolves against the **stated** target, never against `HEAD`, so the **integration-branch** shape needs no `--force`; the **PR-reuse** shape still does, because there the stated target *is* the approved branch — target and branch resolve to the same ref, so the target arm is withheld — and no ancestry evidence exists until the PR squashes). This runs after merges but before doc consolidation
7. **Consolidate documentation** — **first drop any pending-doc whose branch did not actually merge** (`git rev-list --count "<branch>" "^HEAD"`; `0` = merged — and quote the `^` operand, which is a negated glob under zsh `extended_glob` and silently yields the wrong answer unquoted). **A non-zero count is not by itself a verdict:** it is an ancestry test, so a cherry-picked branch scores non-zero even when its content is fully applied — including the `branch: main` pending-doc that Step 4-pre's own `git cherry-pick origin/main..main` produces under `pr` policy. Fall back to `git cherry HEAD "<branch>"` and treat zero unapplied commits as *ask*, not *skip*. Step 4 copies each approved branch's pending-doc to main *before* attempting the merge, so a 4a-SKIP'd branch leaves one here — and consolidating it would route its content, flip its `roadmap_ids` to `done`, archive the body and drop the task's lock with the code never merged. Then parse each surviving `sysop/runtime/pending-docs/*.md` (YAML frontmatter, falling back to the legacy markdown format) and route its entries to the shared docs using the type-based routing table (§ 6.6): `PROJECT_STATUS.md` for all types, `CHANGELOG.md` for bugfixes (under `## [Unreleased]` — the Keep-a-Changelog contract shared with `/release`, Phase 222; **this is one of three paths into that file** — Step 4c also routes the §6 rotation and the wide-close consolidation there, independently of type, see § 6.6 and § 8.1), `UI_Iterations.md` for ui-iterations — and `tasks/index.yml` (flip `status: done` + set `completed_date`; `git mv` body to `tasks/archive/`) for **every ID in `roadmap_ids`, regardless of type** (§ 6.6's note; the type-conditional reading was BeanRider ISSUE-0034). The index rewrite is staged by the same code that writes it, the shared docs get one `git add` each, and the commit is verified afterwards — `git mv` stages but `write_text` does not, and `git add` aborts its whole invocation on a single unmatched pathspec, so a half-staged index is the easy failure here. **This step is also where a closed claim's runtime state is reaped** (Phase 236): for every id it just flipped, Step 4c drops the `sysop/runtime/locks/<ID>.lock`, the `sysop/runtime/parked/<ID>__*.md` marker, and the `sysop/runtime/claim/<ID>/` artifact directory that step 4 deliberately left standing. It reaps the **roadmap** half only — its id list comes from `roadmap_ids`, so a `BATCH-<N>` claim is reaped by `close_batch.sh`'s `remove_claim_artifacts()` instead (§ 8.4) — and both halves are gated on the close having landed on `main` in the main checkout, so a `pr`-policy close on an integration branch defers rather than destroying the record of why a park happened. Each removal is reported, including the already-absent case
...
**How it works:**
- `bash sysop/scripts/claim_task.sh <TASK_ID> <BRANCH>` creates a worktree at `${WORKTREE_ROOT:-..}/${WORKTREE_PREFIX:-<project>}-<task-id>/`. Two optional env vars move it: `WORKTREE_PREFIX` renames the leaf, `WORKTREE_ROOT` (Phase 262) relocates the parent directory — for a sandboxed harness that must declare a writable path and would otherwise have to declare the repo's whole parent. `WORKTREE_ROOT` must name an existing, writable directory that is **not inside any git working tree** — not this repository's and not a neighbour's (widened at Phase 264: the reason a worktree must not go there is that it becomes untracked content the checkout's own cleanup paths would sweep, and that reason does not care which checkout it is). A sandbox directory that legitimately sits inside a larger checkout must therefore be pointed somewhere outside it. The script refuses every other case rather than creating or polluting anything. **`batch_work.sh` honours it too** (Phase 264): a review-batch worktree goes to `${WORKTREE_ROOT:-..}/${WORKTREE_PREFIX:-<project>}-batch-<N>/`, so a sandbox declares one directory for both claim paths rather than one for tasks and the repo's whole parent for batches. Unset, both paths are the historical `../<project>-<leaf>/` exactly
...
- `bash sysop/scripts/claim_task.sh --commit-claim <TASK_ID>` (Phase 261, `Q-397`) commits the forward claim, and is the **only** mode of this script that commits. Under the tracker write mutex it resolves the default branch, applies `_shared/main-push-guard.md` Rule A against the **primary** checkout's `HEAD`, ensures the task is `in_progress` — **re-flipping it if a concurrent claim clobbered Step 4a's uncommitted edit** — and commits `tasks/index.yml` and only that path. `/claim-task` Step 4d and — since Phase 263 (`Q-404`) — `/auto-build` Step 5.4 are its callers. It replaced an inline `git add` + `git commit`, which exited 0 over a task still `open` at `HEAD` in 6 to 25 of 100 concurrent trials in the tight 4a→4d window (0 of 100 with 4b and 4c in between); the same harness measures 0 of 100 in both windows after. A refusal (mutex held, wrong branch, task absent or not claimable) writes nothing and says which
...
| Start new work | `/claim-task <TASK-ID>` | Checks the lock first, then builds in a *fresh* worktree — so the **code** it writes cannot collide with yours. It is **not** read-only on the default branch: Step 4d commits the task's `status: in_progress` flip to `tasks/index.yml` there, in the primary checkout, under Rule A's `HEAD` assert — one small write, and a `--resume` claim skips even that. It is the collision surface, not the build; see the `tasks/index.yml` ceiling below. (`--plan-only` is a different path: it commits the plan and the release flip there too, and starts no work.) |
...
- **`tasks/index.yml` write races — the `/claim-task` path is now serialized, and it is the only one that is.** `/claim-task` Step 4d commits through `claim_task.sh --commit-claim`, which takes the tracker write mutex and re-flips a task whose Step 4a edit a rival clobbered; `claim_task.sh --release` takes the same mutex. Before that, two concurrent claims left one task `open` at `HEAD` while its own claim process exited 0 — 6 to 25 of 100 trials for two claims starting together with nothing between Steps 4a and 4d, and 0 of 100 for the same pair with 4b and 4c in between, which is the window an operator's own typing opens. The rate is load- and window-dependent; the loss is not. 0 of 100 in both windows after. **Still unserialized — three writers, and `Q-404` is the filing:** `/review-close` Step 4c, `backfill_completed_dates.py` and `clear_user_action.py` all write this file without the mutex. All three already write via tempfile + replace, and Phase 263 converted `/auto-build` Step 5.1 — the last one that **truncated in place** — so **no `yaml.safe_dump` code path is a torn-read source any more**. Read that scope narrowly: it covers the seven paths that rewrite the file through an interpreter. It does **not** cover the skills that author the index through the harness — `/intake`'s bulk emit and `/add-task`'s `Edit` append — which truncate at the tool level and were never in that population. What stays unlocked on the claim side is the *pre-commit* edit, `/claim-task` Step 4a and `/auto-build` Step 5.1: both are atomic, and both have a lost flip *repaired* downstream by `--commit-claim` rather than prevented, which is a different property from serialization. A manual `/claim-task` running alongside an `/auto-build` pre-claim now serializes at the commit, where before Phase 263 it did not. The window is seconds (the pre-claim step), not the long build phase.
- **The review-batch tracker has a write mutex — and it does not cover every writer.** The *review-batch* substrate does have a real lock (`sysop/runtime/tracker.write.lock`): `sysop/scripts/batch_work.sh` takes it to claim or release a batch, and `sysop/scripts/close_batch.sh` takes it on a real (non-`--dry-run`) close. A second writer waits up to 120 seconds — `SYSOP_TRACKER_LOCK_WAIT` overrides it — and then **refuses, having written nothing** — the refusal names what *that* caller would have lost, which differs (a batch claim loses a flip, a branch, a worktree and a lock; a close loses a flip, a commit and a lock release). **A lock left behind by a killed process is not broken automatically,** and that is deliberate rather than an oversight: a claim's critical section includes a network `git pull`, so a live holder on a slow remote and a dead one look alike, and breaking it blind restores the lost update the lock exists to prevent. The refusal prints the holder — pid, script, what it was doing, since when — and the exact `rm -f` to run **once you have checked that process is gone**. **Read the scope narrowly.** The lock covers the *batch* writers above — not every writer of `review_tasks.md`. `archive_review_tasks.py`, `/review-close`'s pending-tasks save, and the review skills' own round appends all write that file and take no lock. **`tasks/index.yml` now has the same mutex on the claim path** — `claim_task.sh`'s `--commit-claim` and `--release` take it — and both, like `/claim-task` Step 4a and (since Phase 263) `/auto-build` Step 5.1, now write the index atomically — which is why a stale lock refuses a task claim as well as a batch claim and a close — but the other writers of that index named in the bullet above do not. Treat this as two ceilings raised, not the class closed.
...
| `## Plan review` | *(optional; default: ask)* `always`, `never`, or `ask`. Tells `/claim-task` Step 6 how much the human is in the loop on a claim: `always` → option **A** (a human gate between the adversarial plan review and implementation), `never` → option **B** (run it unattended), `ask` → prompt. Resolved *before* any sub-agent spawns, because planning plus review is 5–25 minutes and asking afterwards asks someone who may have walked away. `--review-plan` / `--no-review-plan` / `--plan-only` override it per run. **There is deliberately no value for option C** (plan-only): C stops before implementation and releases the claim, so a project defaulting to it would never implement anything — it is a per-run choice only. An unrecognised value falls through to the prompt rather than being guessed. See the template below. |
...
"$BASE"...HEAD` to the file types it cares about and invokes lint or
...
not the pre-merge one, where it names the work being shipped. **`$BASE` is
resolved, never the literal `origin/main`** (`Q-380`): on a `master`- or
`develop`-default repository a hard-coded `origin/main` resolves to no ref,
the filter comes back empty, and *empty passes* — so the ratchet reports
green over every changed file in the branch. That is the one place in this
template where a wrong answer is indistinguishable from a clean one, which is
why the snippet resolves the name and **fails** rather than defaulting. Boy-scout
...
```bash
# Resolve the diff base FIRST. `default_branch.sh` prints one bare name and
# exits 0, or prints nothing, explains itself on stderr and exits 1 — and a
# ratchet that cannot name its base must fail, not pass over everything.
# Prefer the remote tip, fall back to the local branch (a repo with no remote
# has no `origin/<name>`, and that is the SAME empty-filter-passes hole the
# hard-coded name was), and refuse when neither ref exists.
#
# WHAT THIS DOES NOT COVER, stated rather than left to be discovered: on a
# no-remote repo under `direct` merge policy, the merge target IS the local
# default branch, so at `4a-post` the range `$BASE_REF...HEAD` is empty by
# construction and the refusal above cannot fire — the local ref exists. The
# ratchet then passes over the assembled merge. That is pre-existing (the
# hard-coded form passed there too) and it is why `4a-post`'s own
# "an empty list here is a contradiction, stop on it" rule is the thing
# actually guarding that case, not this snippet.
BASE=$(bash sysop/scripts/default_branch.sh) || exit 1
BASE_REF="origin/$BASE"
git rev-parse --verify --quiet "$BASE_REF" >/dev/null || BASE_REF="$BASE"
git rev-parse --verify --quiet "$BASE_REF" >/dev/null \
  || { echo "ratchet: no ref for '$BASE' — refusing to pass over an unfiltered diff" >&2; exit 1; }

# Python — lint changed .py files (swap pyflakes for ruff/flake8/mypy/pyright
# depending on your stack)
CHANGED_PY=$(git diff --name-only "$BASE_REF"...HEAD | grep -E '\.py$' || true)
[ -z "$CHANGED_PY" ] || python -m pyflakes $CHANGED_PY

# TypeScript/JavaScript — eslint on changed files under <frontend>/
CHANGED_TS=$(git diff --name-only "$BASE_REF"...HEAD | grep -E '^<frontend>/.*\.(ts|tsx|js|jsx)$' || true)
...
**The first `### Always` line is the deterministic pre-scan, and it is shipped rather than
left to the reader (`Q-353`).** The promoted checks — `checks.yml` regexes, semgrep rules, the
LSP and coverage stages — are the loop's own product, and until they run at a merge gate they
run only at audit time, which is *after* the branch that violated one has landed. The line is
concrete among placeholders on purpose: a consumer who read `docs/getting-started.md` and
believed the close ran the deterministic checks had no way to learn they had to add it, and
the one real project that enforced them at the merge (GDP) added it **by hand**.

**It is not redundant with `/review-close`'s own pre-scan, because the two run on different
trees.** `4a-post` runs the pre-scan on the **assembled** merge target regardless of what this
section says, and de-duplicates against this line so a consumer who lists it does not pay for
it twice. What this line buys is the pass `4a-post` structurally cannot make: `/claim-task`
Step 7e and `/auto-build`'s execution agent run `### Always` **in each branch's own worktree,
at its own tip**, so a violation is caught before the merge rather than in the assembled
result, where it arrives mixed with everyone else's work.

**Cost, stated so the trade is yours to make.** This is a whole-tree scan with no diff
scoping — none exists, and none is needed at the merge gate, because the baseline *is* the
diff signal and Phase 135's accounting already separates a new finding from a baselined one.
In a per-branch worktree, though, it runs once per branch, so a wide `/auto-build` batch pays
it N times. Narrow it (`--mode quality`), move it to the merge gate alone by deleting this
line, or keep it — `/review-close` honours whichever you choose and reports which it took.
...
**`## Plan review` template** *(optional — omit to be prompted each claim)*:
...
> One word: `always` (option **A** — `/claim-task` Step 7d gates on a human
> between the reviewer and the executor), `never` (option **B** — run it
> unattended), or `ask` (the explicit form of the default). The adversarial plan
> reviewer runs on **all three** options; what this setting changes is only what
> happens after the classification at Step 7c, never whether the plan is
> reviewed. Set `never` on a project where you claim tasks and walk away; set
> `always` while you are still calibrating what the planner produces. **Option C
> (plan-only) has no config value on purpose** — pass `--plan-only` on the
> invocation when you want a task planned and reviewed but not implemented. A
> value this file does not recognise is reported and falls through to the
> prompt, because a consumer who mis-spells `always` meant to configure
> something and silently defaulting loses that intent.
...
> **`branch_tip:` is a provenance field, and `/review-close` Step 3b enforces it** (`Q-471`, Phase 282): the collect compares it against the branch tip **in the workspace** and refuses (exit 6) to collect a doc later commits have overtaken — because Step 4c routes `summary:` into `PROJECT_STATUS.md` in the same commit that flips the task `done`, and a doc that no longer describes its branch writes a false statement into the durable record. **Stamp it last**, after the branch's final commit; `date:` cannot serve, being day-granular. A doc without the key is collected and reported under Step 8's `Staleness not measured:` row — never refused — so every doc predating the field still closes.
...
| `tasks/index.yml` + `tasks/{open,deferred,archive}/*.md` | format `tasks/schema.md` | Hybrid task system: YAML index is the source of truth for task metadata (`id`, `status`, `effort`, `blast_radius`, `user_action`, `depends_on`, `body:`, optional `whitelist:`, optional `manual_smoke:`, optional `solo:`). Per-task `.md` bodies hold prose only. `effort` captures how much work; `blast_radius` (Phase 19) captures surface area independently; `manual_smoke: true` (Phase 35, BeanRider ISSUE-0008) declares this task needs a pre-merge human smoke that `/review-close` Step 3c will gate on (procedure under a `## Manual smoke required`-style heading in the body — Phase 218 widened the accepted phrase list and made the declaration itself signal even when no heading matches; see `tasks/schema.md` § Manual smoke). `solo: true` (Phase 235, `Q-235`) declares that the task mutates state shared *outside* the filesystem view — a global lockfile, a singleton registry, a live schema — so `/auto-build` Step 2 batches it alone even when its paths are disjoint from everything else's; it grades serialization, where `blast_radius` grades surface area, and neither implies the other (see `tasks/schema.md` § Solo). `whitelist:` (Phase 27) lets a parent task list IDs that should bypass `/document-work` Step 3b's follow-up-stub check (mirrors the pending-docs frontmatter `whitelist:` bypass; persistent vs. one-off). Validator at `sysop/scripts/validate_tasks.py` enforces 12 invariants — on demand, or at commit time once wired into pre-commit (opt-in; § 8.4) (Invariant 12 is warn-only: when `manual_smoke: true`, body should contain a smoke heading). A 13th, Phase 58b's warn-only test-decision check, was **retired in Phase 234**: it read the body off the filesystem while that record is committed on the feature branch, so from `main` it warned on every claimed task on every run; `/review-close` Step 2d enforces the record at the branch tip instead; `blast_radius` is optional at `schema_version: 1`, required on `open`/`in_progress` at `schema_version: 2`. Invariant 9 (`in_progress requires lock`) resolves the lock path via `git rev-parse --git-common-dir` (Phase 32, 2026-05-22) so every caller — pre-commit from inside a worktree, `/review-close` from main, ad-hoc CLI — finds the same canonical `<main-repo-root>/sysop/runtime/locks/<TASK-ID>.lock`. Replaces the single-file `product_roadmap.md` shape earlier Sysop consumers used. |
...
| `Bash(git reset --hard origin/main)` | `/review-close` Step 6 (`pr` policy — re-sync local `main` after the squash-merge). **The one rule in this table whose text depends on the consumer:** it is an *exact-match* grant, and `install.sh` seeds it with this repository's own default branch, so a `master` consumer's shipped rule reads `origin/master` (Phase 255, `Q-381`). Widening it instead was refused: `origin/:*` does not bind at all (`:*` is only recognized at the *end* of a pattern), and `origin/*` would admit any revision expression after the slash — an allow rule fully authorizes **at the permission layer**, with no escalation there for destructive commands — the auto-mode classifier is a separate, later gate that overrides allow rules for a few known-destructive shapes (`WORKFLOW.md` § 8.2a), which is why the narrowness of this grant is what protects it rather than any escalation. |
...
| `Bash(git add tasks/index.yml)` | `/claim-task` Step 7d's human gate and Step 7f's Option C stage the index directly, and `/review-close` stages it too — subsumed by `Bash(git add:*)`, kept because it is still bound (ground 1 of *When a rule stops being bound* below), not merely because removing a template rule never removes it from an installed consumer. **Not** `/auto-build` Step 5.4: since Phase 263 that step stages and commits inside `claim_task.sh --commit-claim` |
...
| `Bash(bash sysop/scripts/default_branch.sh)` | `/claim-task`, `/auto-build`, `/auto-fix`, `/auto-judge`, `/document-work`, `/release`, and `_shared/main-push-guard.md` + `_shared/ui-verify.md` — every step that compares against or diffs from the default branch resolves its name rather than assuming `main` (Phase 254, `Q-377`). **Bare form only, deliberately:** the skills run it as its own command and read the printed name, because an allow rule does not match past a variable assignment, so a `DEFAULT_BRANCH="$(…)"` capture would bind nothing and route to the auto-mode classifier. |
...
**When a rule stops being bound: delete it.** Ratified 2026-09-08 (Phase 269, `Q-412`). Two rows above justify a *keep* with "removing a template rule never removes it from an installed consumer" — `Bash(git add tasks/index.yml)` and the `archive_review_tasks.py` pair — and that sentence kept getting read as a general reason to retain anything unbound. It is not one. It says the deletion has no effect on **existing** installs — which is a reason not to expect a consumer-side benefit, not a reason to leave the template asserting a binding that does not exist. New installs get the smaller surface, and the row stops lying to the next reader.

The three grounds that *do* justify keeping an unbound rule. Ground 1 has a row above; grounds 2 and 3 are stated in the bullets after the table rather than in rows, and an earlier draft of this sentence claimed all three were rows:

1. **Subsumed and bound elsewhere** — `Bash(git add tasks/index.yml)` is covered by `Bash(git add:*)` and is still bound by `/claim-task` Step 7d and `/review-close`. Unbound *from one skill* is not unbound.
2. **Deliberate operator headroom** — the `archive_review_tasks.py` and `backfill_completed_dates.py` rules exist for a human running the script from a skill session, bound by a documented human recipe rather than by a skill step. Only `archive_review_tasks.py` has a row saying so; the `backfill_completed_dates.py` pair ships with no § 8.2a row at all, which is its own small gap.
3. **A twin of a rule that is bound** — the `.venv/bin/python3` variants (listed in the bullets below the table, not as rows). Note `Bash(python3 -c:*)` is *not* an example of this ground: its own row grounds it as headroom, i.e. ground 2.

A rule matching **none** of the three is deleted, because Phase 152's finding governs: *a rule seeded against invocations that bind none is worse than no rule* — it reads as coverage, and the next reader has to re-derive that it is dead. `Bash(git commit -m claim:*)` was the worked example: bound by nothing after Phase 261 and 263 moved both commits inside `claim_task.sh --commit-claim`, subsumed by nothing (there is no `Bash(git commit:*)`), and headroom for nothing. Two skills carried a bullet explaining why it was kept; deleting it removed the rule, both bullets and this table's row. `Bash(git commit -m rollback:*)` followed one phase later on the same three grounds, when Phase 270 established by execution that the `git commit` in `/auto-build` Step 5's rollback could never have anything to commit and dropped it (`Q-413`) — the rule's only binding.

**Deleting a rule moves the count**, which is load-bearing rather than cosmetic: `install.sh` states the allow-list size in a comment, and `tests/test_upstream_repo_config.py::TestDocs::test_installer_rule_count_comments_track_the_template` derives that number from the template itself, so a deletion that forgets the comment goes red. Update both in the same commit.
...
- `Bash(git commit:*)` — not granted, so the only commit shape the allow-list pre-authorizes is the single seeded `docs:`. (`claim:` and `rollback:` were the other two until Phases 269 and 270 deleted them as unbound — see *When a rule stops being bound* below.) This is narrower than what the skills write: `/auto-fix` and `/auto-judge` document `fix:` / `fix(batch-<N>):` / `revert(batch-<N>):` subjects, which no rule covers and which therefore route to the classifier like any other unlisted call. Neither skill declares a commit rule in its pre-flight, so this is a deliberate gap, not an oversight — but do not describe the one seeded shape as *the* commit shape Sysop uses.
...
- `Bash(git branch -D:*)` — previously omitted as "force-delete; the user should approve interactively." `/review-close` Step 6 under `pr` policy deletes the integration branch and the squash-merged feature branches, neither of which `-d` will drop, so the omission made the documented flow unsatisfiable. The narrower `Bash(git reset --hard origin/main)` (exact string, one ref) joined for the same step — and stays one ref: since Phase 255 the installer substitutes the consumer's default branch into it rather than widening the pattern.
...
- `plan-review-preference.md` — resolves how much the human is in the loop on a claim, read by `/claim-task` Step 6 before any agent spawns. Resolution order is `--review-plan` / `--no-review-plan` / `--plan-only` › `<project>/CLAUDE.md § Plan review` (`always` / `never` / `ask`) › an `AskUserQuestion` fallback, the same "consumer declares its shape" pattern as `§ Merge policy`. Three options on a roadmap task — **A** gates on the human between review and implementation, **B** runs unattended, **C** stops after the review, writes the reviewed plan into the task body and releases the claim — and the adversarial reviewer runs on **all three**; only what follows Step 7c's classification differs. **C is roadmap-only** (a batch has no body file to persist a plan into, so a batch is offered two and `--plan-only` is rejected at Step 1) and has **no tier-2 config value** on purpose: a project defaulting to it would never implement anything. Guided mode pins it to A by overriding the config tier, never the per-run flag. Handles the `askUserQuestionTimeout` interaction: an auto-closed question submits any option the human had already highlighted, so the answer can look entirely ordinary — but the harness also tells Claude the human may be away, and that signal is what the rule keys on (semantically, never on a literal sentence, since the exact wording is not part of the documented interface). Such a result is a **park, never a decision**. Because that is prose an agent follows rather than a lock, the file also names the durable fix: a consumer who sets `askUserQuestionTimeout` should set `§ Plan review`, which removes the question entirely. Lifecycle-only; loop mode does not ship it.
...
| `clear_user_action.py <TASK_ID>` | Clear a task's `user_action` flag once the human-only step is done, so it rejoins the agent-executable frontier all three filters (`/auto-build`, `next_task.py`, `/roadmap`) exclude it from. `--dry-run` previews. Never commits — it hands you the `git add` + `git commit`, exactly as `claim_task.sh` does. Round-trips `tasks/index.yml` through PyYAML with the canonical dump kwargs — it was the **sixth** whole-file writer of that file and is now one of seven — Phase 261 added `claim_task.sh --commit-claim` — and `tests/test_index_writer_class.py` has pinned the kwargs across all of them since Phase 201. That guard counts FILES, not code paths, and `claim_task.sh` now holds two writers, which is why a new mode moves this sentence and not the guard. Before Phase 237 (`Q-314`) nothing cleared the flag, so a task was excluded from every automated path permanently once a human had unblocked it, while `/roadmap` promised a clearing that did not exist |
...
| `close_batch.sh [--merge-target <ref>] <N> [<N2> ...]` | Flip merged batches to `Merged`, check boxes, update stats, and release each closed batch's `BATCH-<N>.lock`. A task annotated `> Failed:` anywhere in its own block — the task line plus the indented lines under it — keeps its checkbox and is excluded from the batch's closed count (and so from the Grand Total) — a FAIL verdict means the work was attempted and not finished, so closing it would overstate what the round resolved; the batch still becomes `Merged`. Accepts `BATCH-<N>` and zero-padded forms. **`--merge-target <ref>` states the branch the close is landing in** (Phase 248, `Q-308`); absent it the target comes from `<project>/CLAUDE.md § Merge policy` — `main` under `direct`, and under `pr` it is left unresolved so only the `main` arm verifies. The target is never inferred from `HEAD`: a branch cut FROM the batch branch contains it while reaching nothing, and a legitimate integration branch is indistinguishable from that on every local ancestry column. The release fires **only when the close committed to `main` in the main checkout** — under `pr` policy Step 4b runs on the integration branch, where stripping the lock would let `/next-task` re-offer a batch whose work is sitting on an unmerged PR; off main it defers loudly and names the recovery. A batch that is already finished (`Merged` / `Complete` / `Ready for Review`) sheds its lock whether or not this run closed anything — otherwise a follow-up re-claim writes a lock every other path refuses to remove. It also removes the batch claim's artifact directory and park marker (`remove_claim_artifacts()`, Phase 236) — **the batch half of close-time reaping, whose roadmap half is `/review-close` Step 4c.** Two owners, because Step 4c's id list is built from `roadmap_ids` only, so no batch id ever reaches it; before Phase 236 a `BATCH-<N>` marker was removed by nothing and accumulated forever. Same landed-on-`main` gate as the lock release, for the same reason, and it reports either way — a removal that is silent when the target is absent cannot be told apart from one that never ran |
...
| `cleanup_worktrees.sh [--clean \| --force]` | List or bulk-reclaim worktrees. Bare = list with a MAIN/MERGED/ACTIVE/STALE classification (not read-only: every mode, this one included, runs `git worktree prune`); `--clean` removes MERGED + STALE and **skips ACTIVE**; `--force` removes **ALL non-main worktrees, ACTIVE included** — uncommitted work is destroyed. Both removing modes also delete the removed worktree's branch with the safe `git branch -d`, so an unmerged branch survives. **ACTIVE is the classifier's fallthrough**, i.e. "not MAIN, not STALE, not merged into main" — it means *may* hold uncommitted work, not *does*: a pristine worktree on an unmerged branch is ACTIVE too. **One arm of it is not a fallthrough, though:** a worktree holding gitignored content under `sysop/runtime/` is classified ACTIVE outright, ahead of the merged check, so `--clean` can no longer reclaim a parked task's `plan.md`/`review.md` or a merged branch's not-yet-collected pending-doc — both are gitignored, so every probe the script had was blind to them, and both shapes reach the MERGED arm because neither branch carries a commit of its own that main lacks (Phase 232). A worktree on an *unmerged* branch was already ACTIVE by fallthrough and is unaffected either way. That probe is scoped to `sysop/runtime/` and no wider on purpose: counting all ignored content would classify nearly every real worktree ACTIVE, since venvs and build output are ignored too, and `--clean` would refuse on the common case. `--force` still removes such a worktree — the fix there is loudness, not a new skip — and names that loss on its own line, separately from the uncommitted-work warning, because a worktree can hold either, both, or neither. **Takes no path operand and has no lock handling:** every mode acts on the whole worktree set, so it is never the way to drop one orphan, and a worktree it removes leaves its `sysop/runtime/locks/<ID>.lock` behind. For one worktree use `git worktree remove <path>` — which refuses on uncommitted or untracked changes, **but not on gitignored ones**: measured in Phase 232, it deletes a worktree's `sysop/runtime/` content silently and exits 0, so it is the weaker door precisely for the content `--clean` now refuses on — or `claim_task.sh --release <TASK_ID>` / `batch_work.sh --release <N>` when a lock and a status also need releasing. A path operand is rejected rather than ignored (Phase 165 — three skill sites had prescribed `--force` as a single-target rollback, so one failed claim would have destroyed every concurrent claim's uncommitted work) |
...
| `scope_overlap.py <TASK_ID>` | Phase 102 collision-aware-claiming primitive. Infers a candidate task's likely scope (`## Key files` + `blast_radius`, a pre-plan guess) and compares it against each in-flight worktree's **actual** changed set (`git diff --name-only <resolved default branch>...HEAD` + uncommitted → lock `files_impacted:` → body fallback; the base is resolved per workspace, and an unresolvable one degrades to uncommitted-only with a note naming that workspace, `Q-380`), emitting a per-in-flight `likely`/`possible`/`none` verdict (text or `--json`). **Advisory, non-blocking** — exits 0 on every degrade path (no in-flight work, missing index, absent PyYAML, unreadable worktree) so it can never break the claim flow. Consumed by `/claim-task` Step 2 (Leg A advisory), `/auto-build` Step 1 (Leg B — soft de-prioritize + Step 4 annotation, Phase 103), `/next-task --avoid-inflight` (opt-in collision ranking, Phase 103), and `/roadmap --in-flight` (portfolio-level candidate + `Run it:` annotation, Phase 103). |
| `security_partition.py` | Phase 231 residual assignment for `/security-audit` Step 3-0c — the files **no** map section reaches. The per-category dispatch is untouched; this covers what it structurally cannot, because 30 of the 36 shipped `security_map` sections are all-placeholder and bind nothing until a consumer localizes them (on a fresh four-pack install the audit reaches six infrastructure sections and not the application code). Ownership is keyed on the **manifest**, never on map sections: `git ls-files` minus the mapped set — **never** minus `CLAUDE.md § Map coverage exclusions`, which scopes the Step 2a coverage audit only (§ 6.1) and is counted here rather than subtracted — grouped by directory for reviewer coherence and packed into `--budget` bins. **Assignment is not coverage** — each agent is handed at most `--per-agent` files and credited for exactly that, with the remainder reported as unreached and the verdict rendered `Full`/`Sampled` on Step 5b's `Residual:` line (**not** its `Coverage:` line — different populations). Sysop's own installed files (read from the lock's `managed_paths`, never a hardcoded prefix) are **counted, not excluded**, and ranked last so a binding budget spends on the consumer's code first. Advisory — exits 0 on every degrade path (no map, no `CLAUDE.md`, empty repo). Ships in both modes. |
...
| `backfill_completed_dates.py` | One-time migration helper — when a project moves from a single-file `[x]` checklist (e.g. `product_roadmap.md`) to the hybrid `tasks/` system, already-done tasks lose their completion timestamps; this reconstructs `completed_date` by grepping git history for the commit that introduced each `[x]` marker. `--dry-run` previews. Not needed after migration: `/review-close` writes `completed_date` on every close. Atomic-rewrite + defensive-parse hardened in Phase 108; Phase 271 (`Q-442`) moved its temp name off `<index>.tmp` to `tempfile.mkstemp` — it was the last writer of `tasks/index.yml` deriving a temp name from its target — which also carried in the symlink write-through and mode preservation the converted siblings had, and added a non-`OSError` cleanup arm the fixed name had not needed (a fixed-name leak self-heals on the next run; a `mkstemp` leak does not). |
| `default_branch.sh [<checkout>]` | Prints the repository's default branch NAME (`main`, `master`, `develop`, …) on stdout, or exits 1 with a diagnostic naming the git command that settles it. The one entry point for `_git_lib.sh`'s `resolve_default_branch`, added in Phase 254 for the SKILL layer: a skill body cannot source a shell library, and Claude Code's permission matcher does not match an allow rule past a variable assignment, so `DEFAULT_BRANCH="$(…)"` would bind no rule. Skills run it bare and substitute the printed name for `<default branch>`. The lifecycle scripts do not use it — they source the library directly. |
| `_git_lib.sh` | Private shared helper (underscore-prefixed — sourced, never executed; the first sourced shell helper in the tree, on the maintainer's call at Phase 251's open). `resolve_default_branch [<checkout>]` prints the repository's default branch NAME (`origin/HEAD` when the branch it names exists locally; else exactly one of `main`/`master`; else an unborn repository's `HEAD` name; else a refusal naming `git remote set-head origin --auto`) and `require_default_branch` is the same with a caller-naming line for scripts that cannot proceed without it. Sourced by `batch_work.sh`, `close_batch.sh`, `cleanup_worktrees.sh` and — since Phase 261 — `claim_task.sh`, each of which fails loud when it is missing rather than falling back to a literal `main` — the `Q-365` defect, where a `master`-default consumer had every batch refused as unmerged, every claim left holding a lock, and every worktree classified ACTIVE. Twin: `sitrep_survey.resolve_default_branch()`, pinned behaviourally — public since Phase 255, because `scope_overlap.py` imports it instead of carrying a third copy (`Q-380`). It also holds the **tracker write mutex**: `tracker_lock_acquire <locks_dir> <what> [<consequences>]` creates `sysop/runtime/tracker.write.lock` with `set -C` (an atomic create-if-absent), waits `SYSOP_TRACKER_LOCK_WAIT` seconds (default 120) for a rival holder, and on timeout refuses with the holder's identity rather than breaking the lock; `tracker_lock_release` removes it, but only if this process is the recorded holder, and is idempotent so it is safe to leave armed in an `EXIT` trap. It serializes the *batch* writers of `review_tasks.md` — `batch_work.sh`'s batch claim and release, and a real `close_batch.sh` close — and, since Phase 261 (`Q-397`), the claim-path writers of `tasks/index.yml`: `claim_task.sh --commit-claim` (the commit half of `/claim-task` Step 4d and, since Phase 263, of `/auto-build` Step 5.4) and `claim_task.sh --release`. It does **not** reach every writer of either file — `archive_review_tasks.py` and the review skills' round appends take no lock on `review_tasks.md`, and three writers of `tasks/index.yml` take none either (`/review-close` Step 4c, `backfill_completed_dates.py`, `clear_user_action.py` — `Q-404`). **Neither `/claim-task` Step 4a nor `/auto-build` Step 5.1 takes a lock, and neither is covered by this:** both write atomically, so neither is a torn-read source, and both have their uncommitted flip *repaired* downstream by `--commit-claim`'s re-flip, which is a different property from serialization. § 4.4 has the full picture. |
