...
3. **Claim the task** — create an isolated git worktree on a feature branch (via `sysop/scripts/claim_task.sh`). `/claim-task` Step 4 always passes `--lock` so the validator's `in_progress requires lock` invariant is satisfied; the lock lives under `<main-repo-root>/.locks/<TASK_ID>.lock` regardless of which worktree invokes the script (Phase 32). Solo workflows that bypass `/claim-task` and call the script directly may omit `--lock`
