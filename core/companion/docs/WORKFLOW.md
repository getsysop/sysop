...
| `clear_user_action.py <TASK_ID>` | Clear a task's `user_action` flag once the human-only step is done, so it rejoins the agent-executable frontier all three filters (`/auto-build`, `next_task.py`, `/roadmap`) exclude it from. `--dry-run` previews. Never commits — it hands you the `git add` + `git commit`, exactly as `claim_task.sh` does. Round-trips `tasks/index.yml` through PyYAML with the canonical dump kwargs — it is the **sixth** whole-file writer of that file, and `tests/test_index_writer_class.py` has pinned the kwargs across all of them since Phase 201. Before Phase 237 (`Q-314`) nothing cleared the flag, so a task was excluded from every automated path permanently once a human had unblocked it, while `/roadmap` promised a clearing that did not exist |
