...
| `Bash(git add tasks/index.yml)` | `/claim-task` Step 7d's human gate and Step 7f's Option C stage the index directly — subsumed by `Bash(git add:*)`, kept because skills name it and removing a template rule never removes it from an installed consumer. **Not** `/auto-build` Step 5.4: since Phase 263 that step stages and commits inside `claim_task.sh --commit-claim` |
| `Bash(git commit -m claim:*)` | **Bound by no skill step since Phase 263.** `/claim-task` Step 4d (since Phase 261) and `/auto-build` Step 5.4 (since Phase 263) both commit inside `claim_task.sh --commit-claim`, so the message shape is unchanged but git no longer runs from skill prose. Retained per the row above rather than deleted; `Q-412` is the retirement filing |
