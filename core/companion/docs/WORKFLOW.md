...
**Inline waivers — `no-check: <id>`.** A grep finding on a line carrying `no-check: <check id>` is suppressed for that check. This is the grep-stage counterpart to `# nosemgrep` and `# pragma: no cover`, and it **composes with the baseline rather than replacing it**: `.claude/checks_baseline.txt` is `file:line`-keyed and rots when a line moves, so it records *historical debt* — or, on an advisory check, a triage verdict that rots the same way — while an inline marker records a *reviewed, intentional exception* that travels with the line it excuses. Where an exception is genuinely permanent, the inline marker is the durable half of that pair whatever the check's `blocking` value.
...
| `.claude/checks_baseline.txt` | auto-generated | Pre-scan baseline — every check's accepted findings except coverage (seeded by `run_checks.sh --update-baseline`) |
