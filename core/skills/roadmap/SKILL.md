...
- **`state`** — one of `pending (not claimed)`, `claimed, no branch`, `empty batch`, `parked`, `parked, work in progress`, `awaiting approval`, `in progress`, `ready for /review-close`. **Every one of these needs git** (branch existence, the main-repo lock dir, and `Doc-Work:` trailers on `git log <branch> --not <default> origin/<default>` — *not* `[x]` checkboxes), which is why the base path cannot produce it and must not pretend to. **`parked` and `awaiting approval` are new (`Q-317`, Phase 248) and were ADDED to this enum, not substituted into it** — the reshape a consumer would have had to absorb was avoided deliberately, so a reader that passes `state` through is unaffected and only one that switches exhaustively on the old five needs a default arm. Their evidence lands in the existing `notes` array. They mean what the task-path states of the same names mean: a claimed batch with 0 commits ahead whose `/claim-task` run stopped at a park marker, or at Step 7d's unanswered human gate. **`parked, work in progress` (`Q-362`, Phase 252) is added the same way**: a claimed batch with ≥1 commits ahead whose park is the latest event on it — the batch committed, then parked. A batch whose newest commit is *younger* than its park marker resumed after parking and reports `in progress` (or `ready for /review-close`) with the marker named in `notes`; it is not a park.
...
| `parked` | **no actuator** — the claim stopped on a human decision. Print the park marker's name from the payload's `notes` and the `next_action`'s `/claim-task BATCH-<N> --resume <RUN_ID>`; do not route it to a drainer, which would claim a batch that already holds a lock. |
| `parked, work in progress` | **no actuator** — same as `parked`, and say that the batch has commits: the `notes` carry the count and the marker. Not `ready for /review-close` even when every task carries a trailer — the park is the latest event and needs the human first. |
| `claimed, no branch` | **no actuator** — the batch holds a lock and its recorded branch does not exist. Relay the `next_action`, which names `batch_work.sh --release <N>`; a drainer would claim a batch that already holds a lock. When the payload's `notes` name a park marker, say so — the batch parked and then lost its branch (`Q-363`). |
| `awaiting approval` | **no actuator** — `/claim-task` Step 7d's gate was reached and not answered. Same shape as `parked`: relay the `--resume` line, route nothing. |
