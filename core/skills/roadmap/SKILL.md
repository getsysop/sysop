...
- **Portability.** The base path is pure file reads (`Read` on three files) — it runs on any agent, including bash-installer/non-Claude consumers, with no git or script dependency. `--in-flight` is the only path that needs `git` + `sysop/scripts/sitrep_survey.py`, and it degrades gracefully to the index-only view when they're absent.
