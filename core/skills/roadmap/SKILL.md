...
- **`state`** — one of `pending (not claimed)`, `claimed, no branch`, `empty batch`, `parked`, `awaiting approval`, `in progress`, `ready for /review-close`. **Every one of these needs git** (branch existence, the main-repo lock dir, and `Doc-Work:` trailers on `git log main..<branch>` — *not* `[x]` checkboxes), which is why the base path cannot produce it and must not pretend to. **`parked` and `awaiting approval` are new (`Q-317`, Phase 248) and were ADDED to this enum, not substituted into it** — the reshape a consumer would have had to absorb was avoided deliberately, so a reader that passes `state` through is unaffected and only one that switches exhaustively on the old five needs a default arm. Their evidence lands in the existing `notes` array. They mean what the task-path states of the same names mean: a claimed batch with 0 commits ahead whose `/claim-task` run stopped at a park marker, or at Step 7d's unanswered human gate.
