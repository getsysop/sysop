...
| 5b       | Any review batch `in progress`                                         | `continue work on batch <N> (branch <branch>) then /document-work`   | no           |
| 6a       | Any task `parked, work in progress`                                    | read the park marker; `--resume <RUN_ID>` only when a run exists — the claim committed before it parked | no           |
| 6b       | Any task `parked`                                                      | read the park marker; `--resume <RUN_ID>` only when a run exists     | no           |
| 6c       | Any task `awaiting approval`                                           | `/claim-task <ID> --resume <RUN_ID>` — approve or revise the plan    | no           |
| 6d       | Any review batch `parked, work in progress`                            | read the park marker; `--resume <RUN_ID>` only when a run exists — the batch committed before it parked | no           |
| 6e       | Any review batch `parked`                                              | read the park marker; `--resume <RUN_ID>` only when a run exists     | no           |
| 6f       | Any review batch `awaiting approval`                                   | `/claim-task BATCH-<N> --resume <RUN_ID>` — approve or revise the plan | no           |
| 6g       | Any task `claimed, no branch`                                          | recreate the branch, or `claim_task.sh --release <ID>`               | no           |
| 6h       | Any review batch `claimed, no branch`                                  | recreate the branch, or `batch_work.sh --release <N>`                | no           |
| 6i       | Any review batch `empty batch`                                         | `verify review_tasks.md batch contents`                              | no           |
| 6j       | Any task `planning`                                                    | `resume planning for <ID>`                                           | no           |
...
**Rows `6a`, `6b`, `6d` and `6e` withhold the `--resume` line when there is no run to resume.** An `/auto-build` park writes a marker and no `sysop/runtime/claim/<ID>/` directory at all, so naming a `<RUN_ID>` there would print a command that cannot work; the recommendation names the marker instead.

**Rows `6a`–`6f` report a stall; they never assert one from absence.** `sysop/runtime/` is gitignored, the `SubagentStop` envelope is Claude-Code-only, and a `--resume` onto a rebuilt worktree legitimately carries no artifacts — so a claim with nothing on disk stays `6j planning`, exactly as before. The probe only ever upgrades a classification on positive evidence (a park marker, or a `classification.md` verdict). Absence is *unknown*, never *did not run*.

**Rows `6d`–`6f` are `6a`–`6c` on the review-batch path (`6e`/`6f` from `Q-317`, Phase 248; `6a`/`6d` from `Q-362`, Phase 252).** `_claim_stall` was always claim-kind agnostic — the runtime paths are keyed by `<CLAIM_ID>`, which is `BATCH-<N>` for a batch — but it had exactly one call site, and `run_survey`'s lock loop `continue`s on a `BATCH-` prefix before reaching it. A `/claim-task` Step-7c park of a review batch therefore reported `in progress — continue work; 0 of N tasks have Doc-Work trailers yet`, which is what an unstarted claim reports. The two classifiers stay separate; the batch one makes its own call under the same rule, so the two paths cannot drift into different answers about what a park is. The batch states are **additive** in `--json`: they are new values of the existing `review_batches[].state` string, with the evidence in the existing `notes` array — no new keys and no reshape, so a consumer that passes the state through is unaffected.

**Rows `6a` and `6d` are the park with work (`Q-362`, Phase 252), and the rule has three arms, applied identically on both paths.** Until Phase 252 the park probe ran only for a claim with 0 commits ahead of the default branch, on the premise that a park is "by construction the state where nothing has been produced" — false for two shipped park sites (`planner-integrity.md` = `VIOLATED` is *defined* as the planner having committed; an executor `STATUS: BLOCKED` parks after the executor ran), so a marker beside one commit reported `in progress`. Now: **0 commits and park evidence → `parked` / `awaiting approval`** (rows `6b`/`6c`, `6e`/`6f`); **commits, and the park is the latest event → `parked, work in progress`** (rows `6a`/`6d`, outranking every commit-shape state including `ready for /review-close`, because a park is a human decision pending and the trailer state of the commits it parked on does not answer it); **commits newer than the park → the claim resumed** and reports its ordinary state, with the marker named in `notes`. The third arm exists because a `--resume` never removes a marker — the close does (`/review-close` Step 4c for a roadmap claim, `close_batch.sh` for a batch), and so does `batch_work.sh --release` for a batch (`claim-task/SKILL.md` § *What cleans it up*) — so between a `--resume` and the close a live claim carries a marker beside its new commits, and calling that `parked` would send the human back to a marker they already acted on. "Latest event" is the later of the marker's write time and the UTC stamp in its name (the run's start, which bounds the park from below), against the newest commit's author date; when neither can be read, the park stands.

**Rows `6g`–`6i` are diagnostics, and until Phase 252 their states had no row and no arm (`Q-363`).** A state a classifier emits with no arm in the cascade drops off `RECOMMENDED NEXT` *and* the ordered list together — `Q-019`'s round recorded that shape for `code committed, docs pending`, and `Q-363` reproduced it for a parked batch whose branch had been deleted: it classified `claimed, no branch` (that arm precedes the park arm on purpose, since a branchless claim is the more actionable diagnosis and `--resume` cannot work without the branch), its park notes were dropped, and it vanished. The fix keeps the ordering, carries the park evidence into `notes`, and gives the state an arm; the same sweep gave `empty batch`, batch `in progress` (row `5b`) and `stale` (row `6k`) theirs. `tests/test_sitrep_park_wip.py` derives the state set from both classifiers and drives one of each through both tables, with no allowlist.
...
The survey script classifies each discovered task into exactly one state. Listed here so consumers reading the output know what each label means. **Review batches have their own vocabulary** — enumerated in `roadmap/SKILL.md`'s `--json` `state` field, and actuator-keyed in its § *Batch actuators* table — but since `Q-317` it shares this table's **Parked**, **Parked, work in progress** and **Awaiting approval** signals, read with `<CLAIM_ID>` = `BATCH-<N>`; rows `6d`–`6f` above route them:
...
| **Ready for `/review-close`** | At least one commit ahead of the default branch carries `Doc-Work: <TASK_ID>`, a pending-doc exists for the branch, and the branch is pushed to origin |
| **Doc-work done, unpushed** | As above with a pending-doc present; local ahead of `origin/<branch>` (or no upstream tracked) |
| **Parked, work in progress** | Lock + branch, ≥1 commits ahead, and park evidence (a marker, or a `verdict: BLOCKED` run) whose time — the later of its write time and the stamp in its name — is **not older than** the newest commit's author date — the claim committed and then parked (`Q-362`). Park evidence *older* than the newest commit means the claim resumed; it takes its ordinary state with the marker named in `notes` |
