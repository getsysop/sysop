...
- ~~**Reviewer-executor collapse for `/auto-build` Phase 6b-6e.**~~ **Closed by events, not deferred — do not re-open it.** This entry proposed porting `/claim-task`'s collapsed reviewer-executor to `/auto-build`, on handoff guidance that said to validate the interactive shape first. Phase 171 settled it the other way: the collapse **failed in the field** and was retired, and `/claim-task` was reshaped to the shape `/auto-build` already had — an independent reviewer (7b), classification at the orchestrator itself (7c), an executor (7e). See `_shared/adversarial-review.md § "The reviewer-executor variant is retired"`, which no longer contains the handoff sentence this entry used to quote. `/auto-build` keeps its three-phase (plan-only → adversarial-reviewer → execution) shape because that shape won.

## First downstream consumer

TBD — Sysop has no consumer with a parallelizable batch yet. The skill ships speculatively against the day a real consumer needs it. The first cycle will surface friction via the consumer's `sysop/SYSOP_ISSUES.md`; iterate from there.
