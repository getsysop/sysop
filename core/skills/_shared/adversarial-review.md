...
> 3. **Silent failure modes** — engine null guards returning empty collections instead of raising; upserts that can overwrite good data with empty/missing values; try/except swallowing the exact error the fix is meant to surface.
...
> -->
>
> Do NOT rewrite the plan. Do NOT suggest stylistic improvements. Return a prioritized list of concrete issues with `file:line` evidence. Under 500 words.
