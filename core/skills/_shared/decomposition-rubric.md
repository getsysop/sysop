...
> **Calibrated 2026-06-17 from curated exemplars across two real task queues (a large multi-service product queue and a small single-user consumer queue).** A rule is stated below as **promotion-grade** only when it surfaced *independently in both* queues — the **cross-queue analogue** of the cross-round-survival gate Sysop uses to promote a convention (WORKFLOW.md §5.1). Not the same gate: §5.1's unit is a later *round within one project*, and this rubric substitutes a second *queue* for that second round. Same principle (one sighting is not evidence), different unit — do not cite §5.1 as defining a cross-project bar. Rules seen strongly in one queue and uncontradicted by the other are marked **candidate**. (These labels grade *the evidence that a rule generalizes* — a separate axis from whether the rubric itself has been wired in and run: the rubric as a whole is **provisional until one real `/intake` dogfood exercises it end-to-end** (Phase 60b), so a rule can be promotion-grade by cross-queue recurrence and still ride on a rubric that's provisional overall.) The archetypes below are generalized shapes, not project identifiers, so they read the same for any consumer.
...
> **Provenance.** Derived in Phase 60b from curated, cross-queue-surviving task exemplars. Cross-queue survival is the promotion gate (the cross-queue analogue of Sysop's convention-promotion semantics — §5.1's unit is a later round within one project, not a second codebase): a sizing or dependency rule earns "promotion-grade" only by appearing independently in two unrelated real queues. The generalized archetypes replace the project-specific task IDs they were distilled from, per the no-project-identifiers authoring rule.
